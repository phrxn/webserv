...
class Config {
	...
	public:
			...
			// Métodos públicos
			std::vector<ServerConfig>		getServers(void); // Retorna o vetor de servidores
};

//config utils functions
namespace ConfigUtils {
	short			getServerCount(const std::string &configFile); // Retorna o número de servidores configurados
	std::string		trim(const std::string &serverBlock ); // Remove espaços em branco do início e do final de uma string
	// bool			hostIsValid(const ServerConfig &server); // Verifica se o host é válido
