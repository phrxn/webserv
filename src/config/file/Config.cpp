...
{
    // Remove espaços em branco desnecessários do bloco de servidor
    std::string trimmedBlock = ConfigUtils::trim(serverBlock);
    // Cria um stream de entrada a partir do bloco de servidor
    std::istringstream serverStream(trimmedBlock);
    // Conjunto para armazenar as chaves do servidor e verificar duplicatas
    ...
    {
        // Verifica se a linha não termina com '[' ou ']' e se está vazia ou não termina com ';'
        if ((line[line.size() - 1] != '[' && line[line.size() - 1] != ']') &&
            (line.empty() || line[line.size() - 1] != ';'))
        {
            // Lança uma exceção se a linha for inválida
        ...
        // Verifica se a chave do token já existe e não é "error_page" ou "location"
        if (tokens[0] != "error_page" && tokens[0] != "location" &&
            serverKeys.find(tokens[0]) != serverKeys.end())
        {
            // Lança uma exceção se a chave for duplicada
            throw std::runtime_error("ERROR_DUPLICATE_KEY");
        }
        else
        {
            // Adiciona a chave ao conjunto de chaves do servidor
        ...
        {
            // Verifica se o token "location" é válido
            if (tokens[1].empty() || tokens.size() != 2 || tokens[1] != "[")
            {
                // Lança uma exceção se o token "location" for inválido
                throw std::runtime_error("ERROR_INVALID_LOCATION");
            }
            else
            {
                // Chama o método _parseRouteStream para processar o bloco de localização
                _parseRouteStream(serverStream, server);
            }
        }
        else
        {
            // Lança uma exceção se a chave for inválida
            throw std::runtime_error("ERROR_INVALID_KEY");
        }
    }
    // Adiciona as configurações do servidor ao vetor de servidores
...
//         if (!locationKeys.insert(tokens[0]).second) {
//             throw std::runtime_error(ERROR_DUPLICATE_KEY);
//         }
